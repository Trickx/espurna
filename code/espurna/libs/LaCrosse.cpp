...
void LaCrosse::Decode2LaCrosseFrame(byte *data, struct Frame *lcframe) {
  if ((data[0] & 0xF0) >> 4 == 9) {
    //struct Frame frame;
    DecodeFrame(data, lcframe);
    if (lcframe->IsValid) {
      lcframe->Text = BuildTextDataString(lcframe);
    }
  }
...
String LaCrosse::BuildTextDataString(struct Frame *frame) {
  // Format
  //
  // OK 9 56 1   4   156 37     ID = 56  T: 18.0  H: 37  no NewBatt
  // OK 9 49 1   4   182 54     ID = 49  T: 20.6  H: 54  no NewBatt
  ...
  if ((frame->Humidity >= 0 && frame->Humidity <= 99)
    ...
    || frame->Humidity == 234) {
    pBuf += frame->NewBatteryFlag ? 129 : 1;
    _bat = frame->NewBatteryFlag ? 129 : 1;
    pBuf += ' ';
  }
  else if (frame->Humidity == 125 || frame->Humidity == 253) {
    pBuf += 2 | frame->NewBatteryFlag ? 130 : 2;
    _bat = frame->NewBatteryFlag ? 130 : 2;
    pBuf += ' ';
  }
  else {
    return "";
...
String LaCrosse::GetTextDataString(byte *data) {
  String fhemString = "";
  ...
  if ((data[0] & 0xF0) >> 4 == 9) {
    ...
    DecodeFrame(data, &frame);
    if (frame.IsValid) {
      fhemString = BuildTextDataString(&frame);
    }
  }
